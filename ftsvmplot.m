...
% text(xp(:,1),xp(:,2),num2str(round(100*sp)/100),'color','b','FontSize',4)%锟斤拷(x,y)锟斤拷写string
% hold on
% h2 = plot(xn(:, 1), xn(:, 2), 'bx','LineWidth',1.5);
% text(xn(:,1),xn(:,2),num2str(round(100*sn)/100),'color','r','FontSize',4)%锟斤拷(x,y)锟斤拷写string
...
h1 = plot(xp(:, 1), xp(:, 2), 'r+','LineWidth',1.5);%正例样本
h2 = plot(xn(:, 1), xn(:, 2), 'b*','LineWidth',1 );%负例样本
h3 = plot(psv(:,1),psv(:,2),'ko','MarkerSize',10);%正例的支持向量
h3 = plot(nsv(:,1),nsv(:,2),'ko','MarkerSize',10);%负例的支持向量
% if ~isempty(ftsvm_struct.NXpv)|~isempty(ftsvm_struct.NXnv)
%     if ~isempty(ftsvm_struct.NXpv)
...
set(h4,'EdgeColor','k');
[C,h] = contour(bigX, bigY, bigZ,[0 0],'k','LineWidth',1.5);%中间的超平面
text_handle=clabel(C,h,'Color','k');
[C,h] = contour(bigX, bigY, bigf1,[0 0],'b:','LineWidth',1.5);%正例的超平面
text_handle=clabel(C,h,'Color','b');
[C,h] = contour(bigX, bigY, bigf2,[0 0],'r:','LineWidth',1.5);%负例的超平面
